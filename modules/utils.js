...
export function trustLine(r){const v=sourceBadge(r)||tr('verified')+' '+(r.last_verified||r.last_checked||''); const score=confidenceScore(r); const conf=score!=null?(score<70?tr('confidenceLow'):tr('confidenceOk',{score})):''; return [v,tr('sourceAvailable'),precisionLabel(r),conf].filter(Boolean).join(' · ')}
