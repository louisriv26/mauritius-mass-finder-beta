...
export function trustLine(r){const dateStr=r.last_verified||r.last_checked||(r.source_last_batch_reconciled_at?r.source_last_batch_reconciled_at.substring(0,7):''); const v=sourceBadge(r)||(dateStr?tr('verified')+' '+dateStr:tr('verified')); const score=confidenceScore(r); const conf=score!=null?(score<70?tr('confidenceLow'):tr('confidenceOk',{score})):''; return [v,tr('sourceAvailable'),precisionLabel(r),conf].filter(Boolean).join(' · ')}
