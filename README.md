...
Typed Near Me searches such as `près de moi`, `pres de moi`, `autour de moi`, `near me`, and `nearby` now trigger the same geolocation workflow as the Near Me button. Results are ranked by soonest upcoming Mass first and distance second. If location is unavailable, the app no longer shows a misleading default nearby result.
