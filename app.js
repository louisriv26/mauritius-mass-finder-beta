...
'use strict';
const APP_VERSION='27.2.7';
const CACHE_NAME='mmf-v27-2-7';
...
function stripStopwords(text){const stop=/\b(this|in|at|on|for|to|i|want|need|find|show|get|please|me|a|an|the|mass|masses|church|churches|du|de|des|le|la|les|l|au|aux|d|en|ce|cet|cette|dans|pour|je|veux|cherche|trouver|montre|moi|une|un|messe|messes|eglise|eglises|near|now|maintenant|proche|proximite|proximité|autour|anglais|english)\b/g;return String(text||'').replace(stop,' ').replace(/\s+/g,' ').trim()}
...
function effectiveDayMode(){
  ...
  if(state.filters.dayMode==='sunday_obligation'&&state.filters.day==='Dimanche'&&effectiveTime()==='afternoon')return 'calendar';
...
function _effectiveDayModeFor(st,p){
  if(st.filters.dayMode==='sunday_obligation'&&st.filters.day==='Dimanche'&&_effectiveTimeFor(st,p)==='afternoon')return 'calendar';
...
function requestLocation(){
  ...
  if(!navigator.geolocation){setState({near:false,nearExpanded:false,nearScope:'',mode:'home'},{url:true});toast(tr('locationDenied'));return}
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    ()=>{locationRequestInFlight=false;setState({near:false,nearExpanded:false,nearScope:'',mode:'home'},{url:true});toast(tr('locationDenied'))},
