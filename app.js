...
function ensureHeaderMenuTopLayer(){
...
function positionHeaderMenu(){
  const menu=$('headerOverflowMenu'), btn=$('headerMenuBtn');
  if(!menu || !btn) return;
  const rect=btn.getBoundingClientRect();
  const width=Math.min(Math.max(220, menu.offsetWidth || 220), window.innerWidth-16);
  const right=Math.max(8, window.innerWidth-rect.right);
  menu.style.right=right+'px';
  menu.style.left='auto';
  menu.style.top=Math.min(Math.max(8, rect.bottom+8), Math.max(8, window.innerHeight-180))+'px';
  menu.style.minWidth=Math.min(224, window.innerWidth-16)+'px';
...
function toggleHeaderMenu(){
  const menu=$('headerOverflowMenu');
  if(!menu) return;
  if(menu.hidden) openHeaderMenu(); else closeHeaderMenu(true);
}
function openAboutPanel(){
  const about=$('aboutPanel');
  if(about){ about.open=true; closeHeaderMenu(true); about.scrollIntoView({behavior:'smooth', block:'center'}); }
}
...
$('menuShareApp')?.addEventListener('click', async()=>{ closeHeaderMenu(true); await shareAppAction(); });
$('headerMenuBtn')?.addEventListener('click', toggleHeaderMenu);
$('menuHelp')?.addEventListener('click',()=>{ closeHeaderMenu(true); openModal('helpModal'); });
$('menuAbout')?.addEventListener('click', openAboutPanel);
document.addEventListener('click', (ev)=>{ const menu=$('headerOverflowMenu'), btn=$('headerMenuBtn'); if(menu && btn && !menu.hidden && !menu.contains(ev.target) && !btn.contains(ev.target)) closeHeaderMenu(false); });
document.addEventListener('keydown', (ev)=>{ if(ev.key==='Escape' && !$('headerOverflowMenu')?.hidden){ ev.preventDefault(); closeHeaderMenu(true); } });
function pinBottomNavToViewport(){
  const nav=document.querySelector('body > .bottomNav');
  if(!nav) return;
...
pinBottomNavToViewport();
window.addEventListener('scroll', pinBottomNavToViewport, {passive:true});
window.addEventListener('resize', ()=>{ pinBottomNavToViewport(); positionHeaderMenu(); }, {passive:true});
if(window.visualViewport){
  window.visualViewport.addEventListener('resize', pinBottomNavToViewport, {passive:true});
  window.visualViewport.addEventListener('scroll', pinBottomNavToViewport, {passive:true});
}
