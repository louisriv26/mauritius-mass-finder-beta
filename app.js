...
async function sharePayload(shareData, fallbackText, copiedMessage){
  if(navigator.share){
    try{
  ...
  try{
    await copyTextFallback(fallbackText || shareData.url || shareData.text || window.location.href);
    if(copiedMessage) showToast(copiedMessage);
  }
...
async function shareAppAction(){
  ...
  await sharePayload({title:tr('shareTitle'), text, url}, url, tr('appShareCopied'));
}
