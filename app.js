...
function handleDelegatedAction(e){
  ...
  const nextAction=e.target.closest('[data-next-action]'); if(nextAction){e.preventDefault(); const a=nextAction.dataset.nextAction; if(a==='locate')requestLocation({force:true}); if(a==='location-dismiss'){setState({near:false,nearExpanded:false,nearScope:'',mode:'home',locationStatus:'',query:removeQueryIntent(state.query,'near')},{url:true});return} if(a==='share'&&state.next)shareResult(state.next); if(a==='save'&&state.next)toggleSave(state.next); return}
