...
function render(){
  ...
  for(const r of displayArr){
    ...
    if(r.qualifier){ const qualifierText = translateLiturgicalText(r.qualifier); badges.appendChild(el('span',{class:'badge'},qualifierText)); }
    if(r.special_rule){ const specialRuleText = translateLiturgicalText(r.special_rule); badges.appendChild(el('span',{class:'badge warn','aria-label':tr('specialRuleBadge')+': '+specialRuleText},specialRuleText)); }
    ...
    if(r.notes && !/source[- ]traced master dataset|freshly re[- ]?scraped/i.test(r.notes)) card.appendChild(el('div',{class:'small'},translateLiturgicalText(r.notes))); 
