...
function filtered(){
  ...
  arr.sort((a,b)=>{
    if(norm(rawQ)){ const sb=searchScoreCache?.get(b) ?? searchScore(b, rawQ), sa=searchScoreCache?.get(a) ?? searchScore(a, rawQ); if(sb!==sa) return sb-sa; }
...
function correctionBody(r){
  const template = [
    'Church / Église: ' + siteDisplayName(r),
    'Town / Ville: ' + (r.town || ''),
    ...
    'Parish / Paroisse: ' + (r.parish_label || r.parish_name || ''),
    'Day/time / Jour/heure: ' + dayLabel(r.day_of_week) + ' ' + r.time_24h,
    'Category / Catégorie: ' + categoryLabel(r),
    'Maps / Cartes: ' + mapsUrl(r),
    'App version / Version de l’application: v' + APP_VERSION
  ];
...
$('q')?.addEventListener('input',()=>{ nextMode=false; quickMode=''; showAllResults=false; render(); renderSearchAssist(true); });
$('q')?.addEventListener('focus',()=>renderSearchAssist(true));
$('q')?.addEventListener('keydown',e=>{ if(e.key==='Enter'){ saveRecentSearch($('q').value); trackEvent('search_used'); renderSearchAssist(false); } });
...
initModals();
window.MMF = {render, filtered, getData:()=>DATA, setLang, version:APP_VERSION, buildMapsQuery, siteDisplayName, searchMatchesRow, searchKey, searchScore, buildSuggestions, isVigilMass, vigilStatus, isSundayFilterMatch, checkForUpdate};
loadData();
})();
