...
const RELEASE_VERIFIED_DATE_EN = '25 April 2026';
const RELEASE_VERIFIED_DATE_FR = '25 avril 2026';
...
const T = {
  en: {
    ...
    qaSundayTitle:'Sunday obligation', qaSundayText:'Show Sunday Masses and Saturday Vigil Masses that count for Sunday.',
    ...
    sundayNotice:'Showing Sunday Masses, including Saturday evening Vigil Masses that satisfy the Sunday obligation.',
  ...
  fr: {
    ...
    qaSundayTitle:'Messe du dimanche', qaSundayText:'Afficher les messes du dimanche et les messes anticipées du samedi soir.',
    ...
    sundayNotice:'Affichage des messes du dimanche, y compris les messes anticipées du samedi soir qui satisfont à l’obligation dominicale.',
...
function detectVigilStatus(r, visible){
  ...
  const text = norm([r.vigil_status,r.special_rule,r.qualifier,r.notes,r.service_type,r.display_category,r.vigil_reason].join(' '));
  if((r.day_of_week || '') === 'Samedi' && (r.is_vigil === true || r.is_vigil_for_sunday === true || text.includes('vigil') || text.includes('anticipee') || text.includes('anticipated'))) return 'explicit';
  return 'not_vigil';
}
...
function renderStats(){
  ...
  $('stats').innerHTML = [[tr('rows'),DATA.length],[tr('parishes'),par],[tr('sites'),sites],[tr('massRows'),masses],[tr('other'),other]].map(x=>'<div class="stat"><b>'+x[1]+'</b>'+x[0]+'</div>').join('') + '<div class="notice ok" style="grid-column:1/-1"><b>'+esc(tr('lastVerified'))+': '+(currentLang === 'fr' ? RELEASE_VERIFIED_DATE_FR : RELEASE_VERIFIED_DATE_EN)+'</b><br><span class="small">'+esc(tr('trustNote'))+'</span></div>'; 
}
...
function isSundayFilterMatch(r){ return r.day_of_week === 'Dimanche' || r.obligation_day === 'Sunday' || isVigilMass(r); }
