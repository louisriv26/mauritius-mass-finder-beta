...
function rel(delta,now=mauritiusNow()){if(delta<=1)return tr('now'); if(delta<60)return tr('inM',{n:delta}); const h=Math.floor(delta/60),m=delta%60; if(delta<1440)return tr('inH',{h,m}); if(delta<2880)return tr('tomorrowRel'); const target=new Date(now); target.setMinutes(target.getMinutes()+delta); return dayName(DAYS[target.getDay()])}
...
function parseQuery(q){
  const n=norm(q); const p={raw:q,tokens:n.split(' ').filter(Boolean),chips:[]}; if(!n)return p;
  ...
  const tm=n.match(/\b(\d{1,2})(?:h|:)?(\d{2})?\s*(pm|am)?\b/);
  if(tm){let h=+tm[1],m=tm[2]?+tm[2]:0;if(tm[3]==='pm'&&h<12)h+=12;if(tm[3]==='am'&&h===12)h=0;if(h<24&&m<60){p.exact=h*60+m;chip('time',`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`)}}
...
function clearIntent(k){
  if(k==='near'){state.near=false;if(state.mode==='near')state.mode='home'}
...
function renderResults(){
  ...
  root.className='resultsGrid'; root.innerHTML=arr.slice(0,60).map(renderCard).join('');
...
function renderMore(){const box=$('#morePanel'); if(state.mode!=='more'){box.hidden=true;box.innerHTML='';return} box.hidden=false; box.innerHTML=`<section class="panel"><h2>${esc(tr('moreTitle'))}</h2><h3 id="helpSection">${esc(tr('helpTitle'))}</h3>${tr('helpHtml')}<h3 id="aboutSection">${esc(tr('aboutTitle'))}</h3><div class="guideText">${tr('aboutHtml')}</div><h3 id="howToUpdateSection">${esc(tr('updateGuideTitle'))}</h3><div class="guideText">${tr('updateGuideHtml')}</div><h3>${esc(tr('trustTitle'))}</h3><p>${esc(tr('trustText'))}</p><p>${esc(tr('version'))}: ${APP_VERSION}</p><button class="btn primary" id="shareAppBtn">${esc(tr('share'))}</button></section>`; $('#shareAppBtn')?.addEventListener('click',shareApp)}
function openMoreSection(sectionId){state.mode='more';render();setTimeout(()=>$(sectionId)?.scrollIntoView({behavior:'smooth',block:'start'}),0)}
function renderFooter(){const f=$('#appFooter'); if(!f)return; f.innerHTML=`<span>${esc(tr('version'))} ${APP_VERSION}</span><span aria-hidden="true">•</span><button type="button" data-footer-target="#helpSection">${esc(tr('helpTitle'))}</button><span aria-hidden="true">•</span><button type="button" data-footer-target="#aboutSection">${esc(tr('about'))}</button><span aria-hidden="true">•</span><button type="button" id="footerUpdateHelp">${esc(tr('howUpdate'))}</button>`; $$('[data-footer-target]').forEach(b=>b.addEventListener('click',()=>openMoreSection(b.dataset.footerTarget))); $('#footerUpdateHelp')?.addEventListener('click',openUpdateHelp)}
function openUpdateHelp(){openMoreSection('#howToUpdateSection')}
...
function requestLocation(){if(!navigator.geolocation){toast(tr('locationDenied'));return} navigator.geolocation.getCurrentPosition(pos=>{state.location={lat:pos.coords.latitude,lon:pos.coords.longitude};state.near=true;state.mode='near';render()},()=>toast(tr('locationDenied')),{enableHighAccuracy:false,timeout:8000,maximumAge:300000})}
...
async function loadData(){state.saved=new Set(JSON.parse(localStorage.getItem('mmf_my_churches')||'[]')); let rendered=false; try{if(window.caches){const cached=await caches.match('data/masses.json'); if(cached){const json=await cached.clone().json(); rendered=applyRows(json); if(rendered)render();}}}catch(e){} if(!rendered){try{rendered=applyRows(window.MMF_FALLBACK_DATA||{}); if(rendered)render();}catch(e){}} try{let res=await fetch('data/masses.json',{cache:'no-cache'}); if(!res.ok)throw new Error('fetch'); let json=await res.clone().json(); if(window.caches){try{const c=await caches.open('mmf-v26-0-1'); await c.put('data/masses.json',res.clone())}catch(_){}} applyRows(json); render(); }catch(e){if(!rendered){state.loadError=String(e); render();}}}
